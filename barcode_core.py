...
def barcode_shapes(code: str, kind: str, scale: float = BARCODE_SCALE) -> ShapeSet:
...
def generate_row(row: InputRow, batch_root: Path, font_path: str, make_svg=True, make_eps=True, make_pdf=True):
    ...
    # SVG giữ bản thô (chưa scale 80%) để bám đúng flow cũ.
    barcode_svg_shapes = barcode_shapes(row.code, row.kind, scale=1.0)
    # EPS/PDF là bản sau action Illustrator, nhỏ hơn SVG theo scale 80%.
    barcode_out_shapes = barcode_shapes(row.code, row.kind, scale=BARCODE_SCALE)

    if make_svg:
        write_svg(batch_root / "svg" / row.kind / f"{names['barcode']}.svg", barcode_svg_shapes, font_path)
    if make_eps:
        write_eps(batch_root / "dist" / row.kind / f"{row.kind}_EPS" / f"{names['barcode']}.eps", barcode_out_shapes, font_path, crop=True)
    if make_pdf:
        write_pdf(batch_root / "dist" / row.kind / f"{row.kind}_PDF" / f"{names['barcode']}.pdf", barcode_out_shapes, font_path)
